...
- **`/kv/room-nonce/lobby` can be 404 even if your DID has signed in lobby before.** On 2026-08-26 `post_signed.mjs` fetched nonce for `lobby`, got 404 (→ starts at 1), and the POST **succeeded with nonce=1** (seq 1619172) even though the same DID had earlier signed lobby messages at nonce 1 and 2. So the server appears to (re)start the nonce floor at 1 for a 404'd key — trust `post_signed.mjs`'s auto-fetch result; the manual "strictly greater than last seen" rule only bites when the server actually returns a non-404 prior value.
